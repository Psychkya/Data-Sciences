...
#check loan grade
loan.third$Loan_Grade[loan.third$Loan_Grade==""] <- "0"
loan.third$Loan_Grade <- as.factor(loan.third$Loan_Grade)
#convert "" in loan sub grade to 0
#loan.third$Sub_Loan_Grade[loan.third$Sub_Loan_Grade==""] <- "0"
...
loan.third.sub <- loan.third[,-which(names(loan.third) %in% c("Loan_Amount_Funded","Loan_Amount_Funded_by_Investors","Sub_Loan_Grade",
...
s2 <- i*n
index <- s1:s2
loan.third.sub <- loan.third.sub[sample(nrow(loan.third.sub)),]
...
rfmodel3 <- randomForest(loan.third.sub.train$Interest_Rate~.,data=loan.third.sub.train[,-1],
                         sampsize=nrow(loan.third.sub.train),replace=TRUE,ntree=101,Proximity=FALSE,importance=TRUE) 

results2 <- predict(rfmodel3, loan.third.sub.test[,-1])
RMSE1 <- sqrt(mean((loan.third.sub.test[,1]) - results2)^2)
RMSE1
SSResidual <- sum((loan.third.sub.test[,1] - results2)^2)
SSTotal <- sum((loan.third.sub.test[,1] - mean(loan.third.sub.test[,1]))^2)
RSquare2 <- 1 - (SSResidual/SSTotal)
RSquare2

loan.third.spec.test <- loan.third.sub[sample(n),]
results2 <- predict(rfmodel,loan.third.spec.test[,-1])
sqrt(mean((loan.third.spec.test[,1]) - results2)^2)
